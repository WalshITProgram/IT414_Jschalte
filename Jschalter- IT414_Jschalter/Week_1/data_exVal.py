...
def main():
    ...
    dollars_regex = re.compile(r'\$[\d,]+\.\d+')
    cc_num_regex = re.compile(r'\b\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b')
...
def validate_dollar(dollar: str) -> bool:
    ...
    """
    dollars_pattern = r'^\$\d{1,3}(,\d{3})*\.\d{2}$'
...
def validate_cc_num(cc_num: str) -> bool:
    ...
    """
    cc_num_pattern = r'^\b\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}\b$'
...
#     assert validate_dollar("$9,782.00") == True
#     assert validate_dollar("$1,000,000.00") == True
#     assert validate_dollar("$0.99") == True
#     assert validate_dollar("$123,456,789.00") == True
#     assert validate_dollar("$100.00") == True
